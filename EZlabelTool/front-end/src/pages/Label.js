...
import { Layout, List, Menu, Button, message, Select, Modal, Typography, Popover } from 'antd';
import { LeftOutlined, RightOutlined, BulbOutlined, AimOutlined, HomeOutlined, BorderOutlined, StarOutlined } from '@ant-design/icons';
...
const guide = (
  <div>
    <p>Objects  : Click the object that you want to label, and start label the image. </p>
    <p>Classifications : After you have finished the label step, please choose the option of classification. </p>
    <p>Skip     : If there is no objects can be labelled, you can click "skip". </p>
    <p>Submit   : Otherwise, please annotate the image and click "submit".</p>
  </div>
);
...
export default function App(props) {
  ...
  const handleNextImage = () => {
    ...
    HttpUtil.post(ApiUtil.API_LABEL_NEXT, values)
      .then(
        re => {
          handleChooseImage();
        }
  ...
  const handleChooseImage = () => {
    ...
    let id = parameters[1];
    let json_info = { project_id: id };
    HttpUtil.post(ApiUtil.API_IMAGE_CHOOSE, json_info)
      .then((result) => {
        if (result.code === 0) {
  ...
  const handleSkip = () => {
    ...
    HttpUtil.post(ApiUtil.API_LABEL_SKIP, values)
      .then(
        re => {
          window.localStorage.previous_status = re.status;
        }
      )
      .then(() => { handleChooseImage(); })
      .catch(error => {
        message.error(error.message);
      });
  };
  ...
  const handleSubmit = () => {
    if (classificationResults.length < classifications.length) {
      message.info("you need to classify this image!");
    } else {
      let values = {
        image_id: image_id,
        project_id: project_id,
      ...
      HttpUtil.post(ApiUtil.API_LABEL_SUBMIT, values)
        .then(
          re => {
            window.localStorage.previous_status = re.status;
          }
  ...
  const setSkipButton = (skipVisible) => {
    if (skipVisible) {
      return (<Button style={{ display: "inline-block", marginRight: "20px" }} onClick={handleSkip}>SKIP</Button>);
    }
  }
      ...
      <Header style={{ display: "flex", justifyContent: "space-between", paddingLeft: "0px", paddingRight: "0px"}}>
        <div style={{ lineHeight: '64px', fontSize: "20px", color: "white"}}>
          ...
          <span style={{ marginLeft: "300px"}}>
            ...
            <Popover content={guide} title="Instructions : " trigger="hover">
              <BulbOutlined style={{ display: "inline-block", margin: "20px" }} type="dashed" shape="circle" />
            </Popover>
            <AimOutlined style={{ display: "inline-block", margin: "20px" }} type="dashed" shape="circle" onClick={() => resetScale()} />
            <RightOutlined style={{ display: "inline-block", margin: "20px" }} type="dashed" shape="circle" onClick={() => nextImage()} />
          </span>
        </div>
        <span>
          {setSkipButton(skipVisible)}
        ...
        <Sider theme="light">
          ...
          <List
            ...
            renderItem={item => (
              <List.Item>
                <List.Item.Meta
                  title={<div style={{ width: 200, height: 20, marginLeft: 15, marginTop: 5 }}>{item.name}</div>}
                  // description={<div style={{ width: 200, height: 20, marginLeft: 15, marginTop: 5 }}>{item.name}</div>}
                />
              </List.Item>
            )}
          />
