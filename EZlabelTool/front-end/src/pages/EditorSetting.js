...
import { Layout, Button, Select, Input, Form, Space, message, Divider, Typography, Popover } from "antd";
...
const guide = (
  <div>
    <p>Click "+ Object" to define different objects with unique name, shape and color. </p>
    <p>Click "+ Classification" to define classifications with unique name, type(single or multiple choice) and options. </p>
    <p>Labelers can use the specific editor template when the project binding it (setup in project menu).</p>
  </div>
);
...
const EditorSetting = (props) => {
  ...
  const addSetting = (values) => {
    ...
    if (!duplicate) {
      ...
      console.log(request)
      HttpUtil.post(ApiUtil.API_EDITOR_SETTING_EDIT, request)
        .then(
          re => {
  ...
  const getData = () => {
    ...
    HttpUtil.post(ApiUtil.API_EDITOR_SETTING, json)
      .then((datasetList) => {
  ...
  return (
    <Layout>
      <Content style={{ background: '#fff', minHeight: 480, maxWidth:750 }}>
        <div style={{ lineHeight: '20px', background: '#fff' }}>
          <Button style={{ display: "inline-block", margin: "20px" }} type="primary" ghost icon={<LeftOutlined />} onClick={backward}>BACK</Button>
          <Title level={4} style={{ display: "inline-block", margin: "10px" }}></Title>
          <Popover content={guide} title="Setting Instructions: Configure the label editor to your exact data requirements." trigger="hover">
            <Button>How to configure an editor?</Button>
          </Popover>
        </div>

        <div style={{ background: '#fff', padding: 24, minHeight: 480, maxWidth:700 }}>
          <Form
            form={form}
            name="dynamic_form_nest_item"
            onFinish={addSetting}
            autoComplete="off"
            style={{ background: '#fff', minHeight: 480, width:700}}
          >
                    ...
                    {fields.map((field) => (
                      <div key={'${field.name}_${field.key}' + field.key}>
                        ...
                        >
                          ...
                          >
                            <Input placeholder="Name" style={{ width: 280 }} />
                          </Form.Item>
                              ...
                              <div>
                                {options.map((option) => (
                                  <Space key={option.key} align="start">
                                    ...
                                    >
                                      <Input placeholder="Option" style={{ width: 445 }} />
                                    </Form.Item>
